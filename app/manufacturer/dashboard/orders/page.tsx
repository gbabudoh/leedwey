    ...
    <div className="p-8 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <div className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2">
            ...
            Factory Execution Manifest
          </div>
          <h1 className="text-4xl font-extrabold text-slate-900 tracking-tight">Sales Orders</h1>
          <p className="text-slate-500 font-medium">Monitor your active production pipelines and payout triggers.</p>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-50 bg-slate-50/50">
                ...
                <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Production</th>
            ...
            <tbody className="divide-y divide-slate-50">
              {orders.map((order) => (
                <tr 
                  key={order.id} 
                  onClick={() => window.location.href = `/manufacturer/dashboard/orders/${order.id}`}
                ...
                >
                  ...
                  <td className="px-8 py-6 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <button className="px-4 py-2 bg-slate-100 text-slate-600 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-600 hover:text-white transition-all shadow-sm">
                        View Manifest
                      </button>
      ...
      <div className="flex items-center gap-6 p-6 bg-slate-900 rounded-[2rem] text-white overflow-hidden relative group">
        ...
        <p className="text-sm font-medium text-slate-300">
          Factory node payouts are secured by <span className="text-white font-bold">Smart Contract Milestones</span>. Ensure proof of production is uploaded for automated release.
        </p>
      </div>
    </div>
  );
