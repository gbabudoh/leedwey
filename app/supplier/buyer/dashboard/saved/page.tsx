...
export default function SupplierSavedPage() {
  const factories = [
        ...
        <div>
           <h1 className="text-4xl font-black text-slate-900 tracking-tight">Tier-1 Factory Repository</h1>
            <p className="text-sm text-slate-400 font-bold uppercase tracking-widest mt-1 flex items-center gap-2">
               ...
               Verified Material Supply Network
            </p>
        </div>
         ...
         {factories.map((factory) => (
            <div key={factory.name} className="bg-white rounded-[3.5rem] border border-slate-100 shadow-sm hover:shadow-2xl hover:shadow-slate-200 transition-all duration-500 group overflow-hidden flex flex-col cursor-pointer">
               ...
               <div className="p-10 space-y-8 flex-1">
                  ...
                  <button className="w-full py-5 bg-slate-900 text-white font-black rounded-[2rem] hover:bg-blue-600 transition-all shadow-xl shadow-slate-200 flex items-center justify-center gap-3 active:scale-95 cursor-pointer uppercase tracking-widest text-[10px]">
                     Initiate Supply RFQ <ArrowRight className="w-4 h-4 cursor-pointer" />
                  </button>
               </div>
            </div>
         ))}
